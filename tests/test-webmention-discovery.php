...
class DiscoverTest extends WP_UnitTestCase {
 ...
 public function discover_absolute_htmllink() {
  ...
  public function test_discover_absolute_htmllink() {
	...
	public function discover_querystring_htmllink() {
	     $headers = $this->headers( array() );
	     $response = $this->response( 200, 'OK' );
	     $body = '<!DOCTYPE html><html lang="en"><head><link rel="webmention"
		href="http://www.example.com/test/4/webmention?query=yes"></head><body>This is a test</body></html>';
			 return $this->httpreturn( $headers, $response, $body );
	}
